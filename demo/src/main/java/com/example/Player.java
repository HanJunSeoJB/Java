...
public class Player {
    private String csvFilePath = "player.csv";	//데이터베이스 위치(성광)
    ...
    String[] question = new String[12];        // qestion:문제 푼 여부를 저장한 변수 문제 번호 0~12, 0:문제를 안품 1:문제를 품

    //id를 매개변수로 받아 player 인스턴스 변수들에 player데이터를 복사한다.
    public Player(String id) {                                        //나중에 데이터베이스 위치를 찾지 못했을때 예외처리 만들어야함 5.09
        this.id = id;
		try (FileWriter writer = new FileWriter(csvFilePath, true);
			Scanner scanner = new Scanner(new File(csvFilePath))) {                                              //palyer ID를 못찾았을 떄 true 나중에 필요하면 쓸 변수
    ...
    public void playerSave(String id) {                                        //나중에 데이터베이스 위치를 찾지 못했을때 예외처리 만들어야함 5.09
		try (FileWriter writer = new FileWriter(csvFilePath, true);
			Scanner scanner = new Scanner(new File(csvFilePath))) {                                            //palyer ID를 못찾았을 떄 true 나중에 필요하면 쓸 변수 
    ...
    public String getUsername() {                       //username은 로그인할 때 썼던 id임 (준서꺼)
        return this.id;
    }
    ...
    public int getPlayerSum(){  
        return Integer.parseInt(playerSum);             //Integer.parseInt(String) : string을 int로 변환
    }
